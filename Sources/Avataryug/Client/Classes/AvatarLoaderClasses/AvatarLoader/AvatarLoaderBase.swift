...
class AvatarLoaderBase
{
    ...
    func LoadHeadModel(completionHandler:@escaping() -> Void)
    {
        ApiEvents.shared.ShowLoading()
        let loadeconomy = AvatarManagementHandler(ApiBase: GetGetallbucketvertices( platform: AvatarManagementAPI.Platform_getGetallbucketvertices.ios))
        loadeconomy.GetGetallbucketvertices(completionHandler:
        {
            (response) in
            switch response
            {
                case .success(let success):
                    ...
                    {
                        self.firstTime = true;
                        self.LoadHeadModelFromPath(completionHandler:
                        {
    ...
    private func LoadHeadModelFromPath(completionHandler: @escaping () -> Void)
    {
        do
        {
            let sceneSource = try GLTFSceneSource(named: "head_generic_rig_threejs5.0.glb")
    ...
    func GetGender() -> Gender
    {
        ...
        //gender =  UserDetailHolder.shared.userDetail.Genders
        return gender;
    }
    ...
    func LoadHeadwear(modelData : GetEconomyItemsResultDataInner)
    {
        ...
        print("THE HEADWEAR---->>",modelData.displayName)
        ...
        DispatchQueue.main.asyncAfter(deadline: .now() + 0.5)
        {
            if(isSameModelPresent)
            {
                ...
                print("REmoving Head Core------",self.HeadCoreBuck)
              //  DataHolder.shared.RemovePart(economyItem: modelData)
            ...
            if(!isSameModelPresent)
            {
                ...
                DispatchQueue.main.asyncAfter(deadline: .now() + 0.5)
                {
                    ...
                    do
                    {
                      ...
                      //  ApiEvents.shared.ShowLoading()
                        self.LoadModelData(urlstr: artifacts[0].mesh_url!, completionHandler:
                                            {
                            data in
                               ...
                               //     DataHolder.shared.AddCurrentBodyPart(economyItem: modelData)
                                    ...
                                    {
                                        ApiEvents.shared.HideLoading()
                                        print("======LoadHeadwear====",currentNodePoint.childNodes.count)
                                        self.OnQueueNetworkModel();
                                    }
                                }
                                catch
                                {
                                    ...
                                    ApiEvents.shared.HideLoading()
                                   // ApiEvents.shared.ShowNotificationMessage(message: error.localizedDescription)
                                }
                            })
                        })
                    }
    ...
    func CheckMissingAfterModelLoad(modelData :GetEconomyItemsResultDataInner)
    {
        ...
        if (modelData.itemCategory == "Top")
        {
            ...
            {
                if (item.name == "lowerbody_without_foot" || item.name == "lowerbody_till_knee")
                {
                ...
                if (item.name == "lowerbody_foot_both")
                {
                    if (item.childNodes.count > 0)
                    {
        ...
        if (modelData.itemCategory == "Bottom")
        {
            ...
            for item in bodyPoints
            {
                if (item.name == "upperbody_sleeve_short" || item.name == "upperbody_sleeve_full" || item.name == "upperbody_sleeveless")
                {
                    if (item.childNodes.count > 0)
                    {
                        isTOpPresent = true;
        ...
        if (modelData.itemCategory == "Outfit")
        {
            ...
            for item in bodyPoints
            {
                ...
                if (item.name == "upperbody_hand_both")
                {
                    if (item.childNodes.count > 0)
                    {
                        isHandPResent = true;
        ...
        if (modelData.itemCategory == "Footwear")
        {
            ...
            for item in bodyPoints
            {
                ...
                if (item.name == "fullbody_without_head" || item.name == "fullbody_with_head_foot" || item.name == "fullbody_without_head_foot")
                {
                    if (item.childNodes.count > 0)
                    {
    ...
    func AddDefaultBodyPart(model:GLTFSceneSource,modelData: ModelData,completionHandler: @escaping () -> Void)
    {
        ...
        do
        {
            ...
            if(modelData.MainCatID == "Top")
            {
                topNodes = bodyPoint
            }
            if(modelData.MainCatID == "Bottom")
            {
                bottomNodes = bodyPoint
            }

            if(modelData.MainCatID == "Footwear")
            {
                footwearNodes = bodyPoint
            }
            if(modelData.MainCatID == "Handwear")
            {
...
//                if(item.name != nil)
//                {
//                    LoopAllNodes(_bone: _bone, completionHandler:
//                                    { node in
//                        if(node.name == item.name)
//                        {
//                            if(item.animationPlayer(forKey: "rotation") != nil)
//                            {
//                                node.addAnimationPlayer(item.animationPlayer(forKey: "rotation")!,  forKey: "rotation")
//                            }
//                            if(item.animationPlayer(forKey: "translation") != nil)
//                            {
//                                node.addAnimationPlayer(item.animationPlayer(forKey: "translation")!,  forKey: "translation")
//                            }
...
//        }
    ...
    func AddNetworkBodyPart(model:GLTFSceneSource,modelData: GetEconomyItemsResultDataInner,closure:()->Void)
      {
          ...
          {
             // ApiEvents.shared.ShowNotificationMessage(message: "Model Data is not encrypted")
              print("========???\(error.localizedDescription)")
              ApiEvents.shared.HideLoading()
              return
          }
   ...
   //     LoadDdat(cat:  "Top")
    ...
    private func LoadAllDefaulModel()
    {
...
//            DataHolder.shared.defaultModelFromNetwork = true
    ...
    //    CurrentSelectedAvatarDetail.shared.ChangedAvatarColor = UserDetailHolder.shared.userDetail.currentUserSelectedAvatar.avatardata.ColorMeta
        ...
        LoadAllDefaulModelNew(OnComplete:
        {
            ApiEvents.shared.HideLoading()
        })
    }

    public func LoadAllDefaulModelNew(OnComplete:@escaping () -> Void)
    {
        var modeldataList : [ModelData] = []
        var gender = "1"
        if(GetGender() == Gender.Male)
        ...
        OnQueueDefaultModelnew(OnComplete: OnComplete)
    }

    func OnQueueDefaultModelnew(OnComplete:@escaping () -> Void)
    {
        if(defaultModelList.count > 0)
        {
            let modelta = defaultModelList[0]
            defaultModelList.remove(at: 0)
            LoadDefaultModelNew( defaultModel: modelta,OnComplete: OnComplete)
        }
        else
        {
            OnComplete()
    ...
    func LoadDefaultModelNew(defaultModel: ModelData,OnComplete:@escaping () -> Void)
    {
            ...
            LoadModelData(urlstr:defaultModel.GlbPath!, completionHandler:
            {
                ...
                self.AddDefaultBodyPart(model:sceneSource,modelData: defaultModel,completionHandler:
                {
                    ...
                    {
                        self.OnQueueDefaultModelnew(OnComplete: OnComplete)
                    }
                })
            })
...
//                            }
    ...
    private func DownloadNetworkModel(modelData : GetEconomyItemsResultDataInner)
    {
        ...
        {
            LoadTattos(modelData: modelData)
        }
        ...
        {
            LoadHeadFaceWearCategory2D(modelData: modelData)
        }
    ...
    {
        var bucketname = GetBucketName(coreBucket: modelData.coreBucket)
        ...
        if(modelData.itemCategory == "Top")
        {
            if(currentSelectedTop != nil)
            {
                ...
                {
                    //DataHolder.shared.RemovePart(category:  "Outfit")
        ...
        if(modelData.itemCategory == "Outfit")
        {
            if(currentSelectedOutfit != nil)
            {
                ...
                {
                    //DataHolder.shared.RemovePart(category:  "Top")
                    //DataHolder.shared.RemovePart(category:  "Bottom")
                    //DataHolder.shared.RemovePart(category:  "Outfit")
        ...
        if(!isModelPresent)
        {
            ...
            do
            {
               // ApiEvents.shared.ShowLoading()
                let artifacts = try JSONDecoder().decode([ThreeDArtifact].self, from: jsonData)
                LoadModelData(urlstr: artifacts[0].mesh_url!, completionHandler:
                {
                    (data) in
                    ModelDecryptionHandler.shared.GetModelData(data: data,completionHandler:
                    {
    ...
    func LoadBodywearPart(modelData : GetEconomyItemsResultDataInner)
    {
        ...
        if(!isModelPresent )
        {
            ...
            do
            {
                ...
                {
                    ApiEvents.shared.ShowNotificationMessage(message: "Data not available")
                    self.OnQueueNetworkModel();
                }
            }
            catch
    ...
    // This method load different category tattoo of
    private func LoadTattos(modelData : GetEconomyItemsResultDataInner)
    {
    ...
    func OnProcessModelTexture(closure:()->Void)
    {
        ...
        if(lastLoadedTattoos.count > 0)
        {
            ...
            {
                if(tattoo.ItemCategory == "HeadTattoo")
                {
                    HeadMaterial.shared.SetTattooTexture(tatto: UIImage(data: tattoo.tattooTex)!)
                }
                else
                {
                    BodyMaterial.shared.SetTattooTexture(key: tattoo.ItemCategory, texture:  UIImage(data: tattoo.tattooTex)!)
                }
            }
        }
        closure()
    ...
    {
        GetVertex()
        if(modelData.itemCategory == "Eyewear" || modelData.itemCategory == "Mouthwear" || modelData.itemCategory == "Earwear" || modelData.itemCategory == "Eyebrowswear" ||
           modelData.itemCategory == "Facewear" || modelData.itemCategory == "Neckwear" || modelData.itemCategory == "Nosewear" )
        {
                        ...
                        data in
                            ...
                            do
                            {
                                ...
                                if(currentNodePoint != nil)
                                {
                                    ...
                                    {
                                        var blendbucket = false
    //                                    GetMorphtargetnode(_bone: hairModelNode, completionHandler:
    //                                    {
    //                                        node in
    //
    //                                        for i in  0..<(node.morpher?.targets.count)!
    ...
    //                                            for item in headwearModelNode.childNodes
    //                                            {
    //                                                print("HEADWEAR IS PRESENT----->>",item.name)
    //                                            }
    ...
    //                                })
                                ...
                                {
                                    self.isClicked = false;
                                    ApiEvents.shared.ShowNotificationMessage(message: "CAN'T LOAD THIS ITEM")
                                    self.OnQueueNetworkModel();
                                }
                            }
                            catch
                                ...
                                do
                                {
                                 ...
                                 //   DataHolder.shared.AddCurrentBodyPart(economyItem: modelData)
                                    ...
                                    if(self.headwearModelNode != nil)
                                    {
                                        ...
                                        GetMorphtargetnode(_bone: tempnode, completionHandler:
                                        {
                                            node in
                                                ...
                                                for i in  0..<(node.morpher?.targets.count)!
                                                {
                                                    if(self.HeadCoreBuck.contains((node.morpher?.targets[i].name)!))
                                                {
                                                    print("THE NODES BUCKET 111---",node.morpher?.targets[i].name!)
                                                    node.morpher?.setWeight(1.0, forTargetNamed: (node.morpher?.targets[i].name!)!)
                                                }
                                                }
                                        ...
                                        GetMorphtargetnode(_bone: tempnode, completionHandler:
                                        {
                                            node in
                                                ...
                                                for i in  0..<(node.morpher?.targets.count)!
                                                {
                                                    if(self.HeadCoreBuck.contains((node.morpher?.targets[i].name)!))
                                                {
                                                    print("THE NODES BUCKET 333---",node.morpher?.targets[i].name!)
                                                    node.morpher?.setWeight(1.0, forTargetNamed: (node.morpher?.targets[i].name!)!)
                                                }
                                        }
    ...
    //                                    ResetBlendshapes()
                                    ...
                                    {
                                        ApiEvents.shared.HideLoading()
                                        print("===HAIR===",currentNodePoint.childNodes[0].name ?? "NONE")
                                        self.OnQueueNetworkModel();
                                    }
                                }
                                catch
    ...
    func LoadHeadFaceWearCategory2D(modelData : GetEconomyItemsResultDataInner)
    {
            ...
            if(addnew)
            {
                ...
                do
                {
                    ...
                    if(artifacts.count > 0)
                    {
                        ...
                        let getDataTask = URLSession.shared.dataTask(with: url, completionHandler:
                        {
                            data, _, error in
        ...
        if(modelData.itemCategory == "SkinTone")
        {
            ...
            if(addnew)
            {
                ...
                if (!currentSkinToneData.artifacts.isEmpty)
                {
                    ...
                    do
                    {
                        let artifacts = try JSONDecoder().decode([SkinToneArtifact].self, from: jsonData)
                        ...
                        if(artifacts.count > 0)
                        {
                            ...
                            let getDataTask = URLSession.shared.dataTask(with: url, completionHandler: { data, _, error in
                                ...
                                HeadMaterial.shared.SetSkinTonetexture(skinTex:  UIImage(data: data)!)
                                
                                guard let url2 = URL(string:  artifacts[0].body_path! )else{
                                    return
                                }
                                let getDataTask2 = URLSession.shared.dataTask(with: url2, completionHandler: { data, _, error in
                                    guard let data = data ,  error == nil else {
                                        return
                                    }
                                    BodyMaterial.shared.SetSkinTonetexture(skinTex:  UIImage(data: data)!)
                                    //DataHolder.shared.AddCurrentBodyPart(economyItem: modelData)
                                    DispatchQueue.main.asyncAfter(deadline: .now() + 0.5) {
                                        ApiEvents.shared.HideLoading()
                                        self.OnQueueNetworkModel()
